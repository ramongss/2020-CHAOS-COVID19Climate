vmd_pred <- function(data){
  # Data preprocessing ----
  # vmd decomposition
  vmd_decomp <- vmd(
    signal = data$confirmed,
    tol = 1e-6,
    DC = FALSE,
    K = 5
  ) %>% data.frame()
  ...
  # create Observed dfs
  Obs   <- data$confirmed[(lag+1):(dim(data)[1]-lag+2)]
  Obs_train <- Obs[1:cut]
  ...
  for (h in seq(horizon)) {
    ...
    for (m in 1:length(model_list)) {
      ...
      for (c in 1:length(Comp_train[[m]])) {
        if (h == 1) {
          # train
          PTRmo[[m]][,c] <- round(predict(model_matrix[[m,c]], Comp_train[[m]][[c]]))
          
          # test
          PTEmo[[m]][,c] <- round(predict(model_matrix[[m,c]], Comp_test[[m]][[c]]))
        } else {
          # train
          for(p in 1:cut) {
            if(p%%hrz !=1) {
              PTRmo[[m]][p,c] <- round(predict(model_matrix[[m,c]], Comp_train[[m]][[c]][p,]))
          ...
          for(p in 1:(n-cut)) {
            if(p%%hrz !=1) {
              PTEmo[[m]][p,c] <- round(predict(model_matrix[[m,c]], Comp_test[[m]][[c]][p,]))
