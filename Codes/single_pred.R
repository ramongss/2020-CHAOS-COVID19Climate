single_pred <- function(data){
  ...
  data_lag <- data_lag[,c(1,5,9,6:8)]
  
  # rename columns
  colnames(data_lag) <- c('y',paste0('lag', seq(2)), colnames(data)[4:6])
  ...
  for (h in seq(horizon)) {
    ...
    for (m in seq(model_list)) {
      ...
      if (h == 1) {
        # train
        PTRmo[[h]][,m] <- round(predict(models[[m]], x_trainm))
        
        # test
        PTEmo[[h]][,m] <- round(predict(models[[m]], x_testm))
      } else {
        # train
        for(p in seq(cut)) {
          if(p%%hrz != 1) {
            non_zero <- round(predict(models[[m]], x_trainm[p,]))
        ...
        for(p in seq(n-cut)) {
          if(p%%hrz !=1) {
            non_zero <- round(predict(models[[m]], x_testm[p,]))
            if(non_zero < 0){non_zero <- 0}
